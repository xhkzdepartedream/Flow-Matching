...
class Trainer:
    ...
    def train(self, ignore_labels: bool, save_every: int = 10, ckpt_path: Optional[str] = None):
        if ckpt_path is not None:
            self._load_checkpoint(ckpt_path, self.local_rank)
        else:
            self._setup()
        self.model.train()
    ...
    def _load_checkpoint(self, checkpoint_path: str, rank: int, use_ema: bool = True):
        ...
        if use_ema and 'ema_state_dict' in checkpoint:
            ...
            self.logger.info(f"[RANK {rank}] Copied EMA weights to the model for continued training.")
        else:
        ...
        self.logger.info(f"[RANK {rank}] Loaded checkpoint from epoch {checkpoint['epoch']}. Resuming training.")
