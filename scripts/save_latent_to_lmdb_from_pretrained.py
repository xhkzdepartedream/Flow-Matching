...
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,6,7'
import lmdb
import time
...
def save_latents_to_lmdb(model: AutoencoderKL, image_dir: str, output_lmdb_dir: str, compression_method: str = 'none',
    ...
    if local_rank == 0:
        merge_lmdbs(source_dir = output_lmdb_dir, target_path = os.path.join(output_lmdb_dir, 'afhq_latents.lmdb'),
...
if __name__ == '__main__':
    ...
    loaded_model = AutoencoderKL.from_pretrained("../../checkpoints/autoencoderkl/")
    src_dict = "../../.cache/kagglehub/datasets/dimensi0n/afhq-512/versions/1/"
    dst_dict = '../data/'
    save_latents_to_lmdb(
        model = loaded_model,
        image_dir = src_dict,
        output_lmdb_dir = dst_dict,
        compression_method = 'scale',
        batch_size = 16,
        num_workers = 8,
        augment_flip = True
    )
