...
from data_processing.init_dataset import AFHQDataset, transform, CelebaHQDataset
...
def load_config(default_config_path: str = None):
    ...
    Args:
        default_config_path (str, optional): The default path to the configuration file.
                                             Can be overridden by the 'config' command-line argument.

    Returns:
        omegaconf.DictConfig: The merged configuration object.
    """
    cli_conf = OmegaConf.from_cli()
    # Use the provided default_config_path, but allow CLI to override it
    config_path = cli_conf.get("config", default_config_path)
    if config_path is None:
        raise ValueError("A configuration file must be specified either via the 'config' "
                         "command-line argument or the 'default_config_path' parameter.")
...
def show_reconstructions(model, images = None, image_dir = None, num_images = 8, device = 'cuda', title_prefix = ""):
    ...
    if images is not None:
        ...
        dataset = CelebaHQDataset(image_dir, transform = transform)
...
def load_model_from_checkpoint(checkpoint_path, model_type: str, device, model_instance = None, use_ema: bool = False, **kwargs):
    """
    ...
    if model_type.lower() == 'autoencoderkl':
        ...
        if use_ema and 'ema_state_dict' in checkpoint:
            print("Loading EMA weights for model")
            # 对于自定义模型，我们假设EMA状态可以直接应用
            ema_state_dict = checkpoint['ema_state_dict']
            if isinstance(ema_state_dict, dict) and 'shadow_params' in ema_state_dict:
                # 从EMA中提取参数并加载到模型
                shadow_params = ema_state_dict['shadow_params']
                cleaned_state_dict = {}
                for (name, _), param in zip(model.named_parameters(), shadow_params):
                    cleaned_state_dict[name] = param
                model.load_state_dict(cleaned_state_dict)
            else:
                # 如果EMA状态字典格式不符合预期，回退到普通加载
                cleaned_state_dict = remove_module_prefix(loaded_state_dict)
                model.load_state_dict(cleaned_state_dict)
        else:
            cleaned_state_dict = remove_module_prefix(loaded_state_dict)
...
def show_tensor_image(image: torch.Tensor, save_path: str = None):
    """
    展示或保存一张图像（灰度或 RGB），无边框无白底。
    - 自动处理 GPU、requires_grad、[-1,1] 范围；
    - 支持 batch 或单图；
    - save_path: 可选，若指定则保存为图像文件，否则 plt.show()
    """
